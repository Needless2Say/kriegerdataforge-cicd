...
_STATUS_HINTS = {
    207: (
        "triage ran but some GitHub writes failed (partial_link) — check the app logs; " "a re-run links the remainder"
    ),
    400: "the app rejected the request (bad request)",
    401: (
        f"{CRON_SECRET_HEADER} mismatch — the cicd-side secret copy is stale against the " "app's REPORTS_CRON_SECRET (rotate: SECRET_ROTATION.md §8.13a)"
    ),
    409: (
        "a triage batch is already running for this app (PL-134 concurrency guard) — " "not double-fired; re-dispatch later"
    ),
    429: "the app's AI provider rate-limited the run — re-dispatch later",
    503: (
        "endpoint is fail-closed: REPORTS_CRON_SECRET is unset on the app — provision it " "via the terraform env block first"
    ),
...
def parse_cli_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description = (
            "Fire the reports-triage cron endpoint of each selected app " "(scripts/reports_registry.json)."
        ),
