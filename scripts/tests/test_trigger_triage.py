...
class TestSelectEntries:
    ...
    def test_unknown_environment_is_a_hard_error(self, sample_registry):
        sample_registry["apps"] = [e for e in sample_registry["apps"] if e["environment"] != "dev"]
...
class TestFireOne:
    ...
    def test_connection_error_prints_type_name_only(self, capsys):
        self.session.post.side_effect = ConnectionError(f"boom {SECRET_FITNESS} in exception text")
        assert fire_one(self._entry(), SECRET_FITNESS) is False
        out = capsys.readouterr().out
...
class TestRun:
    ...
    def test_dry_run_validates_secrets_but_never_posts(self, sample_registry, secret_env, capsys):
        entries = select_entries(sample_registry, "all", "prod")
    ...
    def test_per_app_failures_aggregate_without_aborting_the_fanout(self, sample_registry, secret_env, capsys):
...
class TestTransportContract:
    ...
    def test_cron_url_handles_base_without_trailing_slash(self):
        assert (_cron_url({"base_url": "https://x.example"}) == "https://x.example/reports/triage/cron")
