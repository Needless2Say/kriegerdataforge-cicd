...
class TestRunBackup:
    ...
    def test_with_url_flag_skips_env_file(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            sys,
            "argv",
            self._argv( "--env", "prod", "--url", "postgresql://user:pass@host/db", "--out-dir", str(tmp_path) ),
        )
        out_path_holder = []
        with patch("shutil.which", return_value = "/usr/bin/pg_dump"), \
    ...
    def test_without_url_loads_env_file(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            sys,
            "argv",
            self._argv( "--env", "prod", "--env-file", str(env_file), "--out-dir", str(tmp_path) ),
        )
    ...
    def test_exits_when_db_backup_url_not_in_env_or_file(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("DB_BACKUP_URL", raising = False)
        monkeypatch.setattr(sys, "argv", self._argv( "--env", "prod", "--env-file", str(env_file) ))
    ...
    def test_exits_when_pg_dump_returns_nonzero(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            sys,
            "argv",
            self._argv( "--env", "prod", "--url", "postgresql://x", "--out-dir", str(tmp_path) ),
        )
    ...
    def test_output_filename_uses_dev_env(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            sys,
            "argv",
            self._argv( "--env", "dev", "--url", "postgresql://x", "--out-dir", str(tmp_path) ),
        )
        out_holder = []
        with patch("shutil.which", return_value = "/usr/bin/pg_dump"), \
             patch("subprocess.run", side_effect = _make_pg_dump_ok(out_holder)):
    ...
    def test_custom_out_dir_is_used(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            sys,
            "argv",
            self._argv( "--env", "prod", "--url", "postgresql://x", "--out-dir", str(custom_dir) ),
        )
        out_holder = []
        with patch("shutil.which", return_value = "/usr/bin/pg_dump"), \
             patch("subprocess.run", side_effect = _make_pg_dump_ok(out_holder)):
    ...
    def test_output_directory_created_if_not_exists(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            sys,
            "argv",
            self._argv( "--env", "dev", "--url", "postgresql://x", "--out-dir", str(nested) ),
        )
        out_holder = []
        with patch("shutil.which", return_value = "/usr/bin/pg_dump"), \
             patch("subprocess.run", side_effect = _make_pg_dump_ok(out_holder)):
