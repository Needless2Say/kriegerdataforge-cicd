...
1 (cicd, additive, keeps `e2e-compose.yml` so the dormant callers still resolve) →
2a/2b/2c (any order) → 3 (delete the now unreferenced workflow).

#### Update (2026-07-07)

Step 3 is **deferred into D-008**. The sweep for the D-007 cleanup found that the
